...
from app.models import EventsLines, Rates
...
class RateService:
    @staticmethod
    def add_rates(
        fetched_data: NewPostInput, db: Session, rates_dict: dict, db_ids_dict: dict
    ):
    ...
    def multiple_lines(
        db: Session, rates_dict: dict[int, RateDetails], db_ids_dict: dict[int, int]
    ):
        rates = []
    ...
    def single_line(
        db: Session,
        rates_dict: dict[int, RateDetails | list[RateDetails]],
        db_ids_dict: dict[int, list[int]],
    ):
        rates = []
...
def pack_rates(
    fetched_data: NewPostInput, lines_db: Union[List[EventsLines], EventsLines]
):
    rates_dict = {}
    db_ids_dict = {}
    index = 0
    ...
    lines = (
        [line.id for line in lines_db] if isinstance(lines_db, list) else [lines_db.id]
    )
