...
from app.services.repeater_service import (select_repeater,
                                           select_repeater_custom)
...
class EventService:
    ...
    def generate_post_lines(db: Session, posting_data: NewPostInput, header_id: int):
        ...
        if isinstance(posting_data.line, List):
            return {
                "status": "error",
                "details": "Lines can't be <List>, expected single object",
            }
    ...
    def _handle_repeated_lines(db: Session, posting_data: NewPostInput, header_id: int):
        ...
        if (
            isinstance(repeats_dict, dict) and repeats_dict.get("status") == "error"
        ):  # Handle error
    ...
    @staticmethod
    def _handle_custom_each_day(
        db: Session, posting_data: NewPostInput, header_id: int
    ):
        """
        ...
        if not isinstance(posting_data.line, List):
            return {
                "status": "error",
                "details": "Line data must be a list in custom_each_day mode",
            }

        if posting_data.repeat:
            repeats_dict, payload = EventService._prepare_repeats_and_payload(
                posting_data
            )
    ...
    def _finalize_lines(
        db: Session,
        posting_data: Union[List[RateDetails], RateDetails],
        lines_db: List[EventsLines],
    ):
        """
