...
def upgrade() -> None:
    op.create_table("subcat", sa.Column("id", sa.Integer(), primary_key=True, autoincrement=True, nullable=False))
    ...
    op.add_column("subcat", sa.Column("created_at", sa.TIMESTAMP(timezone=True), nullable=False, server_default=sa.text('now()')))
    
    op.add_column("events_headers", sa.Column("score", sa.Integer(), nullable=False, default=0))
