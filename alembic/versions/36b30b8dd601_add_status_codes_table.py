...
def upgrade() -> None:
    op.create_table("status_codes", sa.Column("id", sa.Integer(), primary_key=True, autoincrement=True, nullable=False))
    op.add_column("status_codes", sa.Column("name", sa.String(), nullable=False))
    op.add_column("status_codes", sa.Column("created_at", sa.TIMESTAMP(timezone=True), nullable=False, server_default=sa.text('now()')))
    
    op.add_column("events_headers", sa.Column("status", sa.Integer(), nullable=False))
    op.create_foreign_key('fk_status_id', 'events_headers', 'status_codes', ['status'], ['id'])
