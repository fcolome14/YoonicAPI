...
@router.get("/tags", status_code=status.HTTP_200_OK, response_model=schemas.SuccessResponse)
def get_tags(category_id: int, db: Session = Depends(get_db), _: int = Depends(get_user_session), request: Request = None) -> schemas.SuccessResponse:
    ...
    response = db.query(
        models.Tags.id, models.Tags.name, models.Tags.subcat, 
        models.Subcategories.code, models.Subcategories.name
    ).join(
        models.Subcategories, models.Subcategories.id == models.Tags.subcat
    ).join(
        models.Categories, models.Categories.id == models.Subcategories.cat
    ).filter(
...
@router.get("/address-suggestions", status_code=status.HTTP_200_OK, response_model=schemas.SuccessResponse)
@limiter.limit("10/minute")
async def get_address_suggestions(input: str, _: int = Depends(get_user_session), request: Request = None) -> schemas.SuccessResponse:
